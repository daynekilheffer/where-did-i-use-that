const babel = require('babel-core')

const { filter, reduce, each } = require('lodash')

const analyze = (file, finalResult) => {
  const transpiledCode = babel.transformFileSync(file, {
    filename: file,
    babelrc: true,
  })
  const scoirImports = filter(transpiledCode.metadata.modules.imports, item => item.source.startsWith('@scoir'))

  const imports = reduce(scoirImports, (result, record) => {
      const mod = record.source
      result[mod] = result[mod] || {}
      each(record.imported, imported => {
        result[mod][imported] = result[mod][imported] || []
        result[mod][imported].push(file)
      })
      return result
  }, finalResult)
}

module.exports = analyze
